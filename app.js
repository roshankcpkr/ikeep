class App {
  ...
  searchNote(event) {
    ...
    let newNotes = this.notes.filter((ele) => {
      if (
        ele.text.toLowerCase().includes(searchInput) ||
        ele.title.toLowerCase().includes(searchInput)
      ) {
        return ele;
      }
    });
class App{
  ...
  openColorTool(event) {
    if (!event.target.matches("#color-icon")) return;
  ...
  editNote() {
    ...
    this.notes = this.notes.map((el) => {
      return el.id === Number(this.id)
        ? { ...el, title: this.noteTitleValue, text: this.noteTextValue }
        : el;
    });
    this.render();
  }
  ...
  displayNote(noteArray) {
    ...
    if (haveNote) {
      const noteHtml = noteArray
        .map((el) => {
          const lenOfChar = el.text.split(" ");
          const textWord =
            lenOfChar.length > 50 ? lenOfChar.slice(0, 60) : lenOfChar;
          const limitedArray = textWord
            .map((els) => {
              return els;
            })
            .join(" ");
          return ` 
                <div class="note" style="background: ${el.color}" data-id = "${
            el.id
          }">
        ...
        if (haveNote){
            const noteHtml =  noteArray.map((el)=>{
                ...
                <div class="note" style="background: ${el.color}" data-id = "${el.id}">
                    ...
                    <div class="tools">
                        <span class="material-symbols-outlined" id="delete-icon" data-id = "${
                          el.id
                        }">
                                delete
                        </span>
                        <span class="material-symbols-outlined" id="color-icon" data-id = "${
                          el.id
                        }">
                        palette
                        </span>
